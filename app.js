...
async function fetchMarkets() {
    try {
        const response = await fetch(`${CONFIG.API_BASE}/markets?limit=50&active=true`);
        ...
        showError('Failed to load markets. Please check your connection and try again.');
    }
}
...
function updateTable(markets) {
    ...
    tableBody.innerHTML = displayMarkets.map(market => {
        ...
        const yesPrice = market.outcomePrices ? market.outcomePrices[0] : 0.5;
